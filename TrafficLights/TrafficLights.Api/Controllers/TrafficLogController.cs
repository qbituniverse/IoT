...
public class TrafficLogController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> CreateTrafficLog([FromBody] TrafficLog document)
    {
        return Ok(await _trafficLogService.Create(document));
    }

    [HttpPut]
    [Route("id/{id}")]
    public async Task<IActionResult> UpdateTrafficLogById(string id, [FromBody] TrafficLog document)
    {
        document.Id = id;
        return Ok(await _trafficLogService.Update(document));
    }
